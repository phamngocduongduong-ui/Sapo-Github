...
import { Plus, Edit, Trash2, CheckCircle, XCircle, RotateCcw, Search, Warehouse as WarehouseIcon } from "lucide-react";
import { getWarehouses, createWarehouse, updateWarehouse, updateWarehouseStatus, deleteWarehouse } from "./actions";
...
export default function WarehousePage() {
  ...
  const handleStatusUpdate = (id: string, status: string) => {
    startTransition(async () => {
      await updateWarehouseStatus(id, status);
      fetchData();
    });
  };
  ...
  return (
    <div style={{ padding: "2rem", width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
        <div>
          <h1 className="page-title" style={{ margin: 0 }}>🏠 Danh mục Kho hàng</h1>
          <p style={{ color: "#64748b", marginTop: "0.5rem" }}>Quản lý hệ thống kho bãi trong doanh nghiệp</p>
        </div>
        <div style={{ display: "flex", gap: "0.75rem" }}>
          <button className="btn btn-outline" onClick={fetchData}>
            <RotateCcw size={18} style={{ marginRight: "6px" }} /> Làm mới
          </button>
          <button className="btn btn-primary" onClick={openAddModal}>+ Thêm kho mới</button>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="modal-overlay" style={{ display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div className="card" style={{ width: "100%", maxWidth: "500px", margin: "1rem" }}>
            <h3 style={{ marginBottom: "1.5rem" }}>{editingItem ? "Chỉnh sửa kho hàng" : "Thêm kho hàng mới"}</h3>
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div>
                <label style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 600 }}>Mã kho *</label>
                <input type="text" name="code" className="input" required defaultValue={editingItem?.code || ""} placeholder="Ví dụ: K01" />
              </div>
              <div>
                <label style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 600 }}>Tên kho *</label>
                <input type="text" name="name" className="input" required defaultValue={editingItem?.name || ""} placeholder="Ví dụ: Kho Tổng HCM" />
              </div>
              <div>
                <label style={{ display: "block", marginBottom: "0.4rem", fontSize: "0.85rem", fontWeight: 600 }}>Ghi chú</label>
                <textarea name="note" className="input" style={{ height: "80px" }} defaultValue={editingItem?.note || ""} placeholder="Nhập ghi chú kho hàng..."></textarea>
              </div>
              <div>
