...
export default function PaymentProposalClient({
  ...
  // Active tab state: "new" (Tạo mới), "debt" (Chờ duyệt công nợ), "ktt" (Chờ KTT), "director" (Chờ GĐ), "done" (Hoàn thành)
  const [activeTab, setActiveTab] = useState<"new" | "debt" | "ktt" | "director" | "done">("new");
  ...
  const proposalsNew = initialProposals.filter(p => p.status === "Tạo mới");
  const proposalsDebt = initialProposals.filter(p => p.status === "Chờ phê duyệt công nợ");
  const proposalsKTT = initialProposals.filter(p => p.status === "Chờ KTT phê duyệt");
  const proposalsDirector = initialProposals.filter(p => p.status === "Chờ giám đốc phê duyệt");
  const proposalsDone = initialProposals.filter(p => p.status === "Hoàn thành");
  ...
  const selectedProposal = initialProposals.find(p => p.id === selectedProposalId) || null;
  ...
  const handleStatusUpdate = async (id: string, nextStatus: string, actionLabel: string) => {
    if (!confirm(`Bạn có chắc chắn muốn ${actionLabel} đề nghị thanh toán này?`)) return;
  ...
  const handleDeleteProposal = async (id: string) => {
    if (!confirm("Bạn có chắc chắn muốn xóa đề nghị thanh toán này?")) return;
                  ...
                  {selectedProposal.status === "Tạo mới" && (
                    <>
                      <button
                        type="button"
                        className="sapo-btn"
                        onClick={() => handleStatusUpdate(selectedProposal.id, "Chờ phê duyệt công nợ", "trình thanh toán")}
                      >
                        Trình thanh toán
                      </button>
                      <button
                  ...
                  {selectedProposal.status === "Chờ giám đốc phê duyệt" && (
                    <>
                      <button
                        type="button"
                        className="sapo-btn"
                        onClick={() => handleStatusUpdate(selectedProposal.id, "Hoàn thành", "phê duyệt")}
                      >
                        GĐ duyệt
                      </button>
                      <button
